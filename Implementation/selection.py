...
def find_ideal_point(individuals):
    ...
    for ind in individuals:
        ...
        current_ideal = np.min(current_ideal,
                                   np.multiply(ind.fitness.wvalues, -1))
    return current_ideal
def find_ideal_point_modified(individuals):
    'Finds the ideal point from a set individuals.'
    current_ideal = [-1 * np.infty] * len(individuals[0].fitness.values)
    for ind in individuals:
        current_ideal = np.maximum(current_ideal, np.multiply(ind.fitness.wvalues, 1.0))
    return current_ideal.tolist()
...
def find_extreme_points_modified(individuals):
    'Finds the individuals with extreme values for each objective function.'
    return [sorted(individuals, key=lambda ind:ind.fitness.wvalues[o], reverse=True)[-1]
            for o in range(len(individuals[0].fitness.values))]
